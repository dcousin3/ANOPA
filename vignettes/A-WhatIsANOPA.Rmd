...
counting the number of participants per group who experienced and illumination. 
...
In all likelihood, solving Sudokus puts participants in a better mental disposition 
...
variance is expected for extreme proportions and more variance is expected for proportions
...
are actually described over two columns (the number of 1s and the total number of 
...
that the Breath condition is not included in the Sudoku condition, so that we can 
...
the Crosswords condition also differs from the Sudoku conditions. These are the only
...
For some, this notation may seems bizarre, or arbitrary. However, it is 
...
number of trials, led to a theoretical variance exactly equal to $1/(4 \times n + 2)$.
