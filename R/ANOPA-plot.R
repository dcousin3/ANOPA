...
#' @param plotLayout (optional; default "line") How to plot the proportions;
...
#' @param ... Other directives sent to superb(), typically 'plotLayout', 
#'          'errorbarParams', etc.
#'   
#'
...
#' \donttest{anopaPlot(w, plotLayout = "bar")}
#' 
#' # Changing the error bar style
#' \donttest{anopaPlot(w, plotLayout = "bar", errorbarParams = list( width =0.1, linewidth=0.1 ) )}
#' 
...
anopaPlot <- function(w, 
    ...
    if (showPlotOnly) { # generate the plot
        res <- superb::superbPlot( wdata,
            ...
            factorOrder    = facts,
            variables      = tempvars,
            statistic      = "ANOPA::prop",  # the summary statistics defined elsewhere (with namespace to avoid masking)
...
#               purpose       = "difference",     # difference-adjusted CI done in CI.prop
        ...
        res <- superb::superbData( wdata,
            ...
            WSFactors      = wsfact,
            variables      = tempvars,
            statistic      = "ANOPA::prop",  # the summary statistics defined above
            errorbar       = "CI",           # its precision define above
            gamma          = confidenceLevel,
            adjustment     = list( 
#               purpose       = "difference",     # difference-adjusted CI done in CI.prop
